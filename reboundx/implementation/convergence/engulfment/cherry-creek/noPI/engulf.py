...
intervals = np.array([0]).astype(int) # No Parameter Interpolation
engulf_times = np.zeros(intervals.size)
...
# copy slices for no paramter interpolation
start = 482                   # line 482 in m/r.txt
end = 4867                    # line 4868 in m/r.txt
mass_slice = masses[start:end]
rad_slice = radii[start:end]
tau_slice = taus[start:end]
...
for i,interval in enumerate(intervals):
    ...
    ps[0].params["tctl_tau"] = tau_slice[0]
    ps[0].params["Omega"] = 0 # explicitly set to 0 (would be 0 by default)

    # initialize main sim
    Nup = end - start              # no. of param updates
    ts = mtimes[start:end] - T0
    ...
    try:
        for j,t in enumerate(ts):
            ...
            ps[0].m = mass_slice[j]
            ps[0].r = rad_slice[j]
            ps[0].params["tctl_tau"] = tau_slice[j]
...
writetxt(intervals, engulf_times, 'output/engulftimes.txt')
